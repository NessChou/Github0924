...
namespace ACME
{
    ...
    class AcmeDateTimeUtils
    {
        ...
        //取得期間的月數
        public static int MonthSpan(DateTime EndDate, DateTime StartDate)
        {

            int Year = Convert.ToInt32((DateToStr(EndDate).Substring(0, 4).ToString())) -
                       Convert.ToInt32((DateToStr(StartDate).Substring(0, 4).ToString()));

            int Month = Convert.ToInt32((DateToStr(EndDate).Substring(4, 2).ToString())) -
                       Convert.ToInt32((DateToStr(StartDate).Substring(4, 2).ToString())) + 1;

            return Year * 12 + Month;
        }


        //判斷日期是否正確 //格式為 20070101
        //DateTime.TryParse
        ...
        //取得這星期的結束
        public static DateTime GetEndDateOfWeek(DateTime dtDate)
        {
            dtDate.AddDays((double)GetWorkingDayOfWeek(dtDate));
            return dtDate.AddDays((double)(0 - (GetWorkingDayOfWeek(dtDate) - 1))).AddDays(6.0);
        }

        //取得這星期的開始
        public static DateTime GetStartDateOfWeek(DateTime dtDate)
        {
            dtDate.AddDays((double)GetWorkingDayOfWeek(dtDate));
            return dtDate.AddDays((double)(0 - (GetWorkingDayOfWeek(dtDate) - 1)));
        }
