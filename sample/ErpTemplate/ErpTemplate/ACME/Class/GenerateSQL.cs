...
namespace ACME
{
	public static class GenerateSQL
	{
        ...
        //string sql_comCustAccounts = "Update comCustAccounts Set CurReceivable=CurReceivable + {1},LocCurReceivable=LocCurReceivable + {1} From " +
        //   "comCustAccounts A Where A.Flag=1 And A.ID='{0}' And A.CurrID='NTD' And A.SysID=2 if @@RowCount=0 " +
        //   "Insert Into comCustAccounts (ID,CurrID,SysID,CurReceivable,LocCurReceivable) Values ('{0}','NTD',2,{1},{1}) ";


        public static string BuildUpdateInsertSQL(DataTable table,string Key)
        {
            ...
            foreach (DataColumn column in table.Columns)
            {
                ...
                if (column.ColumnName.ToLower() == Key.ToLower())
                {
                    i++;
                    continue;
                }
        ...
        // Creates a SqlCommand for inserting a DataRow
        public static SqlCommand CreateUpdateInsertCommand(DataRow row,string Key)
        {
            DataTable table = row.Table;
            string sql = BuildUpdateInsertSQL(table,Key);
            SqlCommand command = new SqlCommand(sql);
            command.CommandType = System.Data.CommandType.Text;
        ...
        // Inserts the DataRow for the connection, returning the identity
        public static object UpdateInsertDataRow(DataRow row, string connectionString,string Key)
        {
            SqlCommand command = CreateUpdateInsertCommand(row,Key);
